...
class UserAnnouncementObject
{
    ...
    //region Users Data Tables

    public function usersDataTablesWithInput()
    {
        $inputCheck = $this->usersDataTablesInputCheck();

        if ($inputCheck->status == false) {
            return $inputCheck;
        }

        return $this->usersDataTables(
            post('id'),
            post('start'),
            post('length'),
            post('keyword'),
    ...
    public function usersDataTablesInputCheck()
    {
        if (isset($_POST['search']) && isset($_POST['search']['value'])) {
            setPost('keyword', $_POST['search']['value']);
        } else {
        ...
        return InputCheck::checkAll([
            new Input('id', Input::METHOD_POST, 'user', Input::TYPE_INT, 1, 16),
            new Input('start', Input::METHOD_POST, 'input_start', Input::TYPE_INT, 1, 8),
            new Input('length', Input::METHOD_POST, 'input_length', Input::TYPE_INT, 1, 8),
            new Input('keyword', Input::METHOD_POST, 'input_keyword', Input::TYPE_TEXT, 0, 64),
            new Input('orderColumn', Input::METHOD_POST, 'input_order_column', Input::TYPE_INT, 1, 8),
    ...
    public function usersDataTables($userID, $start, $length, $keyword, $orderColumn, $orderDir)
    {
        global $user;
        ...
        $select = Database::select("SELECT user_announcement_id, user_announcement_title, user_announcement_message, user_announcement_created_at, user_announcement_user, (SELECT CONCAT_WS(' ', user_first_name, user_last_name) FROM users WHERE user_id = user_announcement_created_by) as createdBy FROM user_announcements WHERE user_announcement_active = 1 AND user_announcement_user = {$userID} {$querySearch} ORDER BY " . $columns[$orderColumn] . ' ' . $orderDir . ' LIMIT ' . $length . ' OFFSET ' . $start);
        $stats = Database::select("SELECT COUNT(*) as recordsFiltered, (SELECT COUNT(*) FROM user_announcements WHERE  user_announcement_user = {$userID} AND user_announcement_active = 1) as recordsTotal FROM user_announcements WHERE  user_announcement_user = {$userID} AND user_announcement_active = 1 {$querySearch}");

        if ($select->status && $stats->status) {
            return new DataTablesOutput(true, Lang::get('user_announcement_users_data_table_success'), $select->data, $stats->data[0]['recordsTotal'], $stats->data[0]['recordsFiltered']);
        } else {
            return new DataTablesOutput(false, Lang::get('user_announcement_users_data_table_failure'));
        }
